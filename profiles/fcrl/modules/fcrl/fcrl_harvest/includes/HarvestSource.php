...
/**
 * Fcrl Harvest HarvestSource Object.
 *
...
class HarvestSource {
  ...
  public function __construct($machineName) {
    ...
    $query->entityCondition('entity_type', 'node')
      ...
      ->fieldCondition('field_fcrl_harvest_machine_name', 'machine', $machineName);
    ...
    $this->machineName = $harvest_source_emw->field_fcrl_harvest_machine_name->machine->value();

    if (!isset($harvest_source_emw->field_fcrl_harvest_source_uri)) {
      throw new Exception('HarvestSource uri invalid!');
    }
    $this->uri = $harvest_source_emw->field_fcrl_harvest_source_uri->value();

    if (!isset($harvest_source_emw->field_fcrl_harveset_type)) {
      throw new Exception('HarvestSource type invalid!');
    }
    $type_machineName = $harvest_source_emw->field_fcrl_harveset_type->value();
    ...
    $optionals = array(
      'filters' => 'field_fcrl_harvest_filters',
      'excludes' => 'field_fcrl_harvest_excludes',
      'overrides' => 'field_fcrl_harvest_overrides',
      'defaults' => 'field_fcrl_harvest_defaults',
    );
  ...
  public function getCacheDir($create_or_clear = FALSE) {
    $cache_dir_path = FCRL_HARVEST_CACHE_DIR . '/' . $this->machineName;
  ...
  public static function getMigrationMachineNameFromName($machineName) {
    $migration_name = FCRL_HARVEST_MIGRATION_PREFIX . $machineName;
  ...
  public static function getHarvestSourceFromNode(stdClass $harvest_source_node) {
    ...
    return new HarvestSource($harvest_source_node_emw->field_fcrl_harvest_machine_name->machine->value());
  }
  ...
  public function getMigration() {
    ...
    $arguments = array(
      // Group all the harvest migration under the "fcrl_harvest" group.
      // TODO better way to utilize the group feature in fcrl_harvest (?).
      'group_name' => 'fcrl_harvest',
      'fcrl_harvest_source' => $this,
    );
