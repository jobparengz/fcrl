...
/**
 * @file
 * File for fcrl_harvest HarvestSource class.
 *
 ...
 */
class HarvestCache {
  const FCRL_HARVEST_CACHE_PROCESSED = 0;
  const FCRL_HARVEST_CACHE_FAILED = 1;
  const FCRL_HARVEST_CACHE_FILTERED = 2;
  const FCRL_HARVEST_CACHE_EXCLUDED = 4;
  const FCRL_HARVEST_CACHE_DEFAULTED = 8;
  const FCRL_HARVEST_CACHE_OVERRIDDEN = 16;
  ...
  public function getFailed() {
    return array_filter($this->processed,
      function ($processed) {
        return ($processed['flag'] & self::FCRL_HARVEST_CACHE_FAILED);
      });
  }
  ...
  public function getFiltered() {
    return array_filter($this->processed,
      function ($processed) {
        return ($processed['flag'] & self::FCRL_HARVEST_CACHE_FILTERED);
      });
  }
  ...
  public function getExcluded() {
    return array_filter($this->processed,
      function ($processed) {
        return ($processed['flag'] & self::FCRL_HARVEST_CACHE_EXCLUDED);
      });
  }
  ...
  public function getDefaulted() {
    return array_filter($this->getSaved(),
      function ($processed) {
        return ($processed['flag'] & self::FCRL_HARVEST_CACHE_DEFAULTED);
      });
  }
  ...
  public function getOverridden() {
    return array_filter($this->getSaved(),
      function ($processed) {
        return ($processed['flag'] & self::FCRL_HARVEST_CACHE_OVERRIDDEN);
      });
  }
  ...
  public function getSaved() {
    ...
    return array_filter($base,
      function ($base_flag) {
        return !($base_flag['flag'] & (self::FCRL_HARVEST_CACHE_EXCLUDED | self::FCRL_HARVEST_CACHE_FAILED));
      });
  }
  ...
  public function setCacheEntry($cache_id, $flag, $title) {
    if (!isset($this->processed[$cache_id])) {
      ...
      if ($flag == self::FCRL_HARVEST_CACHE_PROCESSED) {
        // Processed means no failure. So we make sure that the failed flag is
        // removed.
        $this->processed[$cache_id]['flag'] &= ~(self::FCRL_HARVEST_CACHE_FAILED);
      }
      else {
  ...
  public function setCacheEntryProcessed($cache_id, $title = NULL) {
    $this->setCacheEntry($cache_id, self::FCRL_HARVEST_CACHE_PROCESSED, $title);
  }
  ...
  public function setCacheEntryFailed($cache_id, $title = NULL) {
    $this->setCacheEntry($cache_id, self::FCRL_HARVEST_CACHE_FAILED, $title);
  }
  ...
  public function setCacheEntryFiltered($cache_id, $title = NULL) {
    $this->setCacheEntry($cache_id, self::FCRL_HARVEST_CACHE_FILTERED, $title);
  }
  ...
  public function setCacheEntryExcluded($cache_id, $title = NULL) {
    $this->setCacheEntry($cache_id, self::FCRL_HARVEST_CACHE_EXCLUDED, $title);
  }
  ...
  public function setCacheEntryDefaulted($cache_id, $title = NULL) {
    $this->setCacheEntry($cache_id, self::FCRL_HARVEST_CACHE_DEFAULTED, $title);
  }
  ...
  public function setCacheEntryOverridden($cache_id, $title = NULL) {
    $this->setCacheEntry($cache_id, self::FCRL_HARVEST_CACHE_OVERRIDDEN, $title);
  }
