...
class FCRLMigrateBaseTestSetup
{
...
class FCRLMigrateBaseTest  extends PHPUnit_Framework_TestCase
{
    public static function setUpBeforeClass()
    {
      // module_enable(array('fcrl_migrate_base_example'), TRUE);
      $setup = new FCRLMigrateBaseTestSetup();
    ...
    public function rollback($migrationName, $remainingNodes = 0)
    {
      ...
      // Test rollback
      // TODO: FCRL comes with 4 resources. Remove first so count is 0.
    ...
    public function migrate($migrationName)
    {
      ...
      fcrl_migrate_base_add_modified_column($table);
    ...
    public function testCKANResourceImport()
    {
      ...
      $this->migrate('fcrl_migrate_base_example_ckan_resources');
...
<p>Original data here:
  <a href="https://data.cityofmadison.com/Polling-Places/Polling-Places/rtyh-6ucr">https://data.cityofmadison.com/Polling-Places/Polling-Places/rtyh-6ucr</a></p>';
    ...
    {
      $expected = $result = array();
      $this->migrate('fcrl_migrate_base_example_ckan_dataset');
    ...
    {
      $this->rollback('fcrl_migrate_base_example_ckan_dataset');
    }

    public function testCKANResourceRollback()
    {
      $this->rollback('fcrl_migrate_base_example_ckan_resources');
    }

    public function testDataJsonImport()
    {
      $expect = $result = array();
      $this->migrate('fcrl_migrate_base_example_data_json11');
      ...
      $expect['status']  = 1;
      if (module_exists('fcrl_workflow')) {
      ...
      $expect['resource2DownloadUrl']  = "https://s3.amazonaws.com/fcrl-default-content-files/files/Polling_Places_Madison_0.csv";
      $result['resource2AccessUrl']  = $resource2->field_link_api['und'][0]['url'];
      $expect['resource2AccessUrl']  = "http://demo.getfcrl.com/dataset/wisconsin-polling-places";
    ...
    public function testDataJsonHighwater() {
      $this->rollback('fcrl_migrate_base_example_data_json11');
      // First run should create some elements and update 0.
      $migration = $this->migrate('fcrl_migrate_base_example_data_json11');
      ...
      $data_folder = implode(
        ...
        array(
          DRUPAL_ROOT,
          drupal_get_path('module', 'fcrl_migrate_base_example'),
          'data',
        )
      );
      // Prepare files
    ...
    public static function tearDownAfterClass() {
      // module_disable(array('fcrl_migrate_base_example'));
      self::setMigrationEndpointName('data_json_1_1', 'data.json');
    }
}

class FCRLMigrateBaseTestUnit extends PHPUnit_Framework_TestCase
{
    public function setup() {
        include "fcrl_migrate_base.module";
    }

    public function testIsoConversion() {
        $date = "2000-01-15T00:45:00Z";
        $result = fcrl_migrate_base_iso_interval_to_timestamp($date);
