...
for i in "${!files[@]}"; do
    if [ $(($i % $CIRCLE_NODE_TOTAL)) -eq $CIRCLE_NODE_INDEX ]; then
      echo "Running ahoy fcrl test `pwd`/${files[$i]} --format=pretty --out=std --format=junit --out=$CIRCLE_ARTIFACTS/junit ${params} --colors"
      time ahoy fcrl test `pwd`/${files[$i]} --format=pretty --out=std --format=junit --out=$CIRCLE_ARTIFACTS/junit ${params} --colors
    fi
