...
require "./fcrl/.ahoy/.scripts/circle-behat-balancer.rb"
require "pp"
...
files[CIRCLE_NODE_INDEX].each_index do |i|
  ...
  testCmd = "ahoy fcrl test #{file} --suite=#{suite} --format=pretty --out=std --format=junit --out='#{CIRCLE_ARTIFACTS}/junit' #{params} --colors"
  if i > 1
