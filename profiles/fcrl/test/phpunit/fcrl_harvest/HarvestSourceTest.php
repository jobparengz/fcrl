...
class HarvestSourceTest extends \PHPUnit_Framework_TestCase {
  ...
  /**
   * Status of fcrl_harvest_test.
   *
   * @var fcrlHarvestTestBeforClassStatus
   */
  ...
  public static function setUpBeforeClass() {
    ...
    if (!module_exists('fcrl_harvest_test')) {
      self::$fcrlHarvestTestBeforClassStatus = FALSE;
  ...
  public function testHarvestSourceConstruct() {
    ...
    $node->field_fcrl_harvest_machine_name[$node->language][]['machine'] = 'test_harvest_source_construct';

    $node->field_fcrl_harvest_source_uri[$node->language][0]['value'] = 'https://data.mo.gov/data.json';
    $node->field_fcrl_harvest_source_uri[$node->language][0]['safe_value'] = 'https://data.mo.gov/data.json';

    $node->field_fcrl_harveset_type[$node->language][]['value'] = 'harvest_test_type';
    ...
    $this->assertEquals($source->type,
      HarvestSourceType::getSourceType($node->field_fcrl_harveset_type[$node->language][0]['value']));
    $this->assertEquals($source->uri, $node->field_fcrl_harvest_source_uri[$node->language][0]['safe_value']);
  ...
  public function testGetCacheDir() {
    $source_remote = $this->getRemoteSource();
    $source_remote_cachedir_path = FCRL_HARVEST_CACHE_DIR .
      '/' .
  ...
  public static function tearDownAfterClass() {
    ...
    $query->entityCondition('entity_type', 'node')
      ...
      ->fieldCondition('field_fcrl_harvest_machine_name', 'machine', 'test_harvest_source_construct');
    ...
    // modules.
    if (!self::$fcrlHarvestTestBeforClassStatus) {
