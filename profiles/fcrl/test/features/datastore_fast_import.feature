...
Feature: FCRL Datastore Fast Import
  Background:
    Given pages:
      | name                | url                    |
      | Datastore Settings  | /admin/fcrl/datastore  |
    Given users:
      | name    | mail                | roles                |
    ...
    When I click "Edit"
    And I click "Upload"
    And I attach the file "fcrl/Afghanistan_Election_Districts_test.csv" to "field_upload[und][0][resup]" using file resup
  ...
  Scenario: As user I want to import files using batch imports
    ...
    Given I am on the resource "Resource Datastore"
     ...
     Then I wait for "FCRL Datastore File: Status"
     When I press "Import"
  ...
  Scenario: As user I want to set fast imports as default for all the resource with a size over a threshold
    Given I am logged in as a user with the "site manager" role
      And I am on "Datastore Settings" page
      And I select the radio button "Use fast import for files with a weight over:"
      And I fill in "fcrl_datastore_fast_import_selection_threshold" with "1KB"
      And I press "Save configuration"
    Given I am on the resource "Resource Datastore"
    When I click "Manage Datastore"
      Then I wait for "FCRL Datastore File: Status"
