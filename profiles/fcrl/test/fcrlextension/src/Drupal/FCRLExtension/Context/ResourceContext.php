 ...
 */
class ResourceContext extends RawFCRLEntityContext{
  ...
  public function externalPreviewsAreEnabledForFormat($provider, $setting, $format_name)
  {
    ...
    $preview_settings = variable_get("fcrl_dataset_format_previews_tid{$format->tid}", array());
    // If $setting was "enabled," the preview is turned on. Otherwise, it's
    // turned off.
    $preview_settings[$provider] = ($setting == 'enabled') ? $provider : 0;
    variable_set("fcrl_dataset_format_previews_tid{$format->tid}", $preview_settings);
  }

    /**
     * Override RawFCRLEntityContext::post_save()
     */
