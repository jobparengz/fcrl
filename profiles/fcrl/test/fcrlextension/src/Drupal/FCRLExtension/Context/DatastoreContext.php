 ...
 */
class DatastoreContext extends RawFCRLContext {
    ...
    foreach ($result as $n) {
      if (!empty($n->nid)) {
        ...
        foreach ($importer_ids as $importer_id) {
          ...
          $has_file = fcrl_datastore_file_field($node);
          $wrapper = entity_metadata_wrapper('node', $node);
          $status = ($has_file) ? FCRL_DATASTORE_FILE_EXISTS : FCRL_DATASTORE_EMPTY;
  ...
  public function assertHasNoDatastoreRecords($title) {
    $nid = $this->getNidByTitle($title);
    $status = fcrl_datastore_records($nid);
    if (!empty($status)) {
  ...
  public function resetDatastoreSettings($scope) {
    variable_set('fcrl_datastore_fast_import_selection_threshold', '20MB');
