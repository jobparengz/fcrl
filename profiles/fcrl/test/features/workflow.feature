# time:5m51.88s
@api @enableFCRL_Workflow @disablecaptcha
Feature:
  Workflow (Workbench) tests for FCRL Workflow Module
  ...
  Scenario: As a user with the Workflow Supervisor role, I should be able to publish stale 'Needs Review' content.
    ...
    And datasets:
      ...
      | Stale Dataset FCRL Test Needs Review  | Contributor  | No        | Jul 21, 2015      | Jul 21, 2015  |
      | Fresh Dataset FCRL Test Needs Review  | Contributor  | No        | Jul 21, 2015      | Jul 21, 2015  |
    And resources:
      | title                                  | author       | dataset                              | format |  published |
      | Stale Resource FCRL Test Needs Review  | Contributor  | Stale Dataset FCRL Test Needs Review | csv    |  no        |
    And I update the moderation state of "Stale Dataset FCRL Test Needs Review" to "Needs Review" on date "30 days ago"
    And I update the moderation state of "Stale Resource FCRL Test Needs Review" to "Needs Review" on date "30 days ago"
    And I update the moderation state of "Fresh Dataset FCRL Test Needs Review" to "Needs Review" on date "20 days ago"
    Given I am logged in as "Supervisor"
    And I visit the "Stale Reviews" page
    And I should see the button "Reject"
    And I should see the button "Publish"
    When I fill in "edit-title" with "FCRL Test"
    And I press "Filter"
    Then I should see "Stale Dataset FCRL Test Needs Review"
    And I should see "Stale Resource FCRL Test Needs Review"
    And I should not see "Fresh Resource FCRL Test Needs Review"
    And I check the box "Select all items on this page"
  ...
  Scenario: Check harvested datasets are published by default even when fcrl_workflow is enabled.
    Given users:
      | name               | mail                     | status | roles             |
  ...
  Scenario: As an anonymous user I should see a revisions link when fcrl_workflow is enabled.
    Given pages:
      | name               | url                                  |
