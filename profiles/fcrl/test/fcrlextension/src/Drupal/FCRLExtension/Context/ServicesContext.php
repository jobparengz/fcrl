 ...
 */
class ServicesContext extends RawFCRLContext {
  private $base_url = '';
  private $csrf_token = '';
  ...
  public function gatherContexts(BeforeScenarioScope $scope) {
    ...
    $this->datasetContext = $environment->getContext('Drupal\FCRLExtension\Context\DatasetContext');
  }
  ...
  public function iUseTheEndpointToCreateTheNodes($endpoint, $data) {
    ...
    foreach ($data->getHash() as $node_data) {
      ...
      if ($response->getStatusCode() == '200') {
        ...
        $this->fcrlContext->entityStore->store('node', $node->type, $node->nid, $wrapper, $wrapper->label());
      }
      else {
  ...
  public function iUseTheEndpointToAttachTheFileTo($endpoint, $file_name, $node_name) {
    // Get node.
    $node = $this->fcrlContext->entityStore->retrieve_by_name($node_name);
    if ($node) {
      // Build file path.
  ...
  private function process_field($field, $field_value) {
    switch ($field) {
      ...
      case 'program code':
        ...
        break;
      case 'resource':
        $resource = $this->fcrlContext->entityStore->retrieve_by_name($field_value);
        if ($resource) {
