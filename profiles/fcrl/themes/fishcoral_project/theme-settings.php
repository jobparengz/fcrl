 ...
 */
function fishcoral_project_form_system_theme_settings_alter(&$form, &$form_state) {
  global $theme;
  // Ensure this include file is loaded when the form is rebuilt from the cache.
  $form_state['build_info']['files']['form'] = drupal_get_path('theme', 'fishcoral_project') . '/theme-settings.php';
  ...
  $copyright = theme_get_setting('copyright', 'fishcoral_project');
  $form['copyright']['copyright'] = array(
    '#title' => t('Footer text'),
    '#type' => 'text_format',
    '#format' => 'html',
    '#default_value' => isset($copyright['value']) ? $copyright['value'] : t('Powered by <a href="http://getfcrl.org/">FCRL</a>'),
  );

  $display_login_menu = (theme_get_setting('display_login_menu', 'fishcoral_project') === NULL) ? 1 : theme_get_setting('display_login_menu', 'fishcoral_project');
  ...
  // Upload field.
  $hero = theme_get_setting('hero_file', 'fishcoral_project');
  ...
  $form['hero']['background_option'] = array(
    ...
    '#required' => FALSE,
    '#default_value' => theme_get_setting('background_option', 'fishcoral_project'),
...
// Hero container padding.
  $form['hero']['hero_padding'] = array(
    ...
    '#required' => FALSE,
    '#default_value' => theme_get_setting('hero_padding', 'fishcoral_project'),
  ...
  $form['logo']['settings']['svg_logo'] = array(
    ...
    '#required' => FALSE,
    '#upload_location' => file_default_scheme() . '://theme/',
    '#default_value' => theme_get_setting('svg_logo', 'fishcoral_project'),
    '#upload_validators' => array(
 ...
 */
function fishcoral_project_site_information_theme_settings_form_submit(&$form, &$form_state) {
 ...
 */
function fishcoral_project_hero_system_theme_settings_form_submit(&$form, &$form_state) {
  if ($form_state['values']['hero_file']) {
    $fid = $form_state['values']['hero_file'];
    _fishcoral_project_file_set_permanent($fid);
  }
 ...
 */
function _fishcoral_project_file_set_permanent($fid) {
  cache_clear_all('fishcoral_project_hero_file_uri', 'cache');
  $file = file_load($fid);
  $file->status = FILE_STATUS_PERMANENT;
  file_save($file);
  file_usage_add($file, 'theme', 'file', $fid);
  fishcoral_file_insert($file);
}

/**
 * Implements hook_file_insert().
 */
function fishcoral_file_insert($file) {
