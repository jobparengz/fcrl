 ...
 * the FCRLContext (with it's methods for manipulating the session page via the
 ...
 * MinkContext.
 *
 * I could potentially just do all this from FCRLContext itself, but that would
 * be at the cost of making that class even more complex than it already is.
 */
class DatasetAutofillContext implements Context {

  /**
   * Compose with instances of ServicesContext and FCRLContext.
   *
   * @BeforeScenario
   */
  public function gatherContexts(BeforeScenarioScope $scope) {
    $environment = $scope->getEnvironment();
    $this->servicesContext = $environment->getContext('Drupal\FCRLExtension\Context\ServicesContext');
  ...
  public function iAutoFillInRequiredFields(TableNode $tableNode) {
    ...
    $this->fcrlContext->getMink()->fillFields($tableNode);
  }
