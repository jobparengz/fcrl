 ...
 */
class DatasetContext extends RawFCRLEntityContext {
  ...
  public function gatherContexts(BeforeScenarioScope $scope) {
    ...
    $this->groupContext = $environment->getContext('Drupal\FCRLExtension\Context\GroupContext');
    $this->fcrlContext = $environment->getContext('Drupal\FCRLExtension\Context\FCRLContext');
  }
  ...
  public function iShouldSeeADatasetCalled($text) {
    ...
    $search_region = $page->find('css', '.view-fcrl-datasets');
  ...
  public function iShouldSeeTheLocalPreviewLink() {
    $this->assertSession()->pageTextContains(variable_get('fcrl_dataset_teaser_preview_label', '') . ' ' . t('Preview'));
  }
  ...
  /**
   * @Given I :operation the :option on FCRL Dataset Forms
   */
  public function iTheOnFcrlDatasetForms($operation, $option) {
    $enabled = 0;
    if ($operation === "enable") {
      $enabled = 1;
    }
    ...
    switch ($option) {
      case 'Strict POD validation':
        variable_set('fcrl_dataset_form_pod_validation', $enabled);
        break;

      case 'Groups validation':
        variable_set('fcrl_dataset_form_group_validation', $enabled);
        break;
