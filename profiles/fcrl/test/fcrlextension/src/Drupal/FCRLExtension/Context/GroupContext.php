 ...
 */
class GroupContext extends RawFCRLEntityContext {
  /** @var  \Drupal\FCRLExtension\Context\FCRLContext */
  protected $fcrlContext;
  ...
  public function iAmAMemberOfTheGroup($role, $group_name) {
    ...
    $account = $this->fcrlContext->getCurrentUser();
  ...
  public function iShouldSeePermissionsForTheGroup(){
    ...
    foreach($permissions as $machine_name => $perm) {
      $this->fcrlContext->getMink()->assertPageContainsText(strip_tags($perm['title']));
    }
  }
  ...
  public function iShouldSeeRolesForGroup($group){
    ...
    foreach($roles as $machine_name => $role) {
      $this->fcrlContext->getMink()->assertPageContainsText(strip_tags($role));
    }
  }
