...
class DatajsonHarvestMigrationTest extends PHPUnit_Framework_TestCase {
  ...
  public function testResources($dataset) {
    $expected_resources = array(
      'TEST - Workforce By Generation (2011-2015)' => 'https://s3.amazonaws.com/fcrl-default-content-files/phpunit/GenChart_0_0.csv',
      'TEST - Retirements (2011 - 2015)' => 'https://s3.amazonaws.com/fcrl-default-content-files/phpunit/retirements_0.csv',
      'TEST - Retirements: Eligible vs. Actual' => 'https://s3.amazonaws.com/fcrl-default-content-files/phpunit/2015EligibleVsActual.csv',
    );
  ...
  public function testMetadataSources($dataset) {
    if (!module_exists('fcrl_dataset_metadata_source')) {
      $this->markTestSkipped('fcrl_dataset_metadata_source module is not available.');
    }
    else {
  ...
  public function testModerationState($dataset) {
    if (!module_exists('fcrl_workflow')) {
      $this->markTestSkipped('fcrl_workflow module is not available.');
    }
    else {
