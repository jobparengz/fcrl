 ...
 */
class WorkflowContext extends RawFCRLContext {
  ...
  /**
   * @BeforeFeature @enableFCRL_Workflow
   */
  public static function enableFCRL_Workflow(BeforeFeatureScope $scope)
  {
    ...
    @module_enable(array(
      'fcrl_workflow',
      'fcrl_workflow_permissions',
      'drafty',
      'workbench_moderation',
      'workbench_email', 'workbench',
      'views_fcrl_workflow_tree',
      'menu_badges',
      'link_badges',
    ));

    if (module_exists('fcrl_feeedback')) {
      features_revert(array('fcrl_feedback'));
    }
  ...
  /**
   * @AfterFeature @enableFCRL_Workflow
   */
  public static function disableFCRL_Workflow(AfterFeatureScope $event)
  {
  ...
  public function iClickTheLinkNextToTitle($link, $title) {
    $items = $this->getSession()->getPage()->findAll('xpath', "//span[contains(@class,'views-fcrl-workflow-tree-title')]/a[text()=' " . $title . "']/../../span[contains(@class, 'views-fcrl-workflow-tree-action')]/a[text()='" . $link . "']");
    if (empty($items)) {
