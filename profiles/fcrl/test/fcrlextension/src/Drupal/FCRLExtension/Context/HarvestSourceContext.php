...
use Drupal\FCRLExtension\Context\RawFCRLEntityContext;
 ...
 */
class HarvestSourceContext extends RawFCRLEntityContext {
  ...
  public static function setupFeature(BeforeFeatureScope $scope)
  {
    ...
    if($feature->getTitle() == 'Fcrl Harvest') {
  ...
  public function theHarvestSourceIsHarvested($machine_name) {
    ...
    // Harvest Cache
    fcrl_harvest_cache_source($harvest_source);
    // Harvest Migration of the test data.
    fcrl_harvest_migrate_source($harvest_source);
  ...
  public function harvestSetup(BeforeScenarioScope $event) {
    ...
    foreach ($sources as $machine_name => $title) {
      ...
      $wrapper->title = $title;
      $wrapper->status = 1;
      $wrapper->field_fcrl_harveset_type = 'datajson_v1_1_json';
      $wrapper->field_fcrl_harvest_source_uri = 'https://s3.amazonaws.com/fcrl-default-content-files/files/data_harvest_test.json';
      $wrapper->field_fcrl_harvest_machine_name = array(
        'human' => $title,
